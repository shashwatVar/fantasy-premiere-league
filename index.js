		...
		function recur(n) {
			User.findOne({name: user[n - 1].name}, {useFindAndModify: false}, (e, u) => {
				if (!e) {
					u.team.push({
						name: user[n - 1].name,
						plforward: user[n - 1].plforward,
						plmidfield: user[n - 1].plmidfield,
						pldefender: user[n - 1].pldefender,
						plgk: user[n - 1].plgk,
					});
					u.team2.push({
						name: user[n - 2].name,
						plforward: user[n - 2].plforward,
						plmidfield: user[n - 2].plmidfield,
						pldefender: user[n - 2].pldefender,
						plgk: user[n - 2].plgk,
					});
					if (u.team.length == 2) {
		...
		if (!e) {
			...
			for (x = 0; x < u.length; x++) {
				if (u[x].team.length != 0) {
					...
					let ate;
					if (
						completed_array.includes(u[x].team[0].name) != true &&
						completed_array.includes(u[x].team2[0].name) != true
					) {
						...
						attacking_rating +=
							(u[x].team[0].plforward[0].Shooting + u[x].team[0].plforward[0].Phyiscality) *
							(u[x].team[0].plforward[0].Morale / 100);
						attacking_rating2 +=
							(u[x].team2[0].plforward[0].Shooting + u[x].team2[0].plforward[0].Phyiscality) *
							(u[x].team2[0].plforward[0].Morale / 100);
						...
						for (j = 0; j < 5; j++) {
							midfield_at_rating +=
								(u[x].team[0].plmidfield[j].Shooting +
									u[x].team[0].plmidfield[j].Dribbling +
									u[x].team[0].plmidfield[j].Passing) *
								(u[x].team[0].plmidfield[j].Morale / 100);
							midfield_at_rating2 +=
								(u[x].team2[0].plmidfield[j].Shooting +
									u[x].team2[0].plmidfield[j].Dribbling +
									u[x].team2[0].plmidfield[j].Passing) *
								(u[x].team2[0].plmidfield[j].Morale / 100);
							midfield_de_rating +=
								u[x].team[0].plmidfield[j].Defending * (u[x].team[0].plmidfield[j].Morale / 100);
							midfield_de_rating2 +=
								u[x].team2[0].plmidfield[j].Defending * (u[x].team2[0].plmidfield[j].Morale / 100);
						...
						for (j = 0; j < 4; j++) {
							defensive_rating +=
								(u[x].team[0].pldefender[j].Defending + u[x].team[0].pldefender[j].Phyiscality) *
								(u[x].team[0].pldefender[j].Morale / 100);
							defensive_rating2 +=
								(u[x].team2[0].pldefender[j].Defending + u[x].team2[0].pldefender[j].Phyiscality) *
								(u[x].team2[0].pldefender[j].Morale / 100);
						}
						...
						net_defensive = defensive_rating * 0.9 + midfield_de_rating * 0.1;
						net_attacking2 = attacking_rating2 * 0.75 + midfield_at_rating2 * 0.25;
						net_defensive2 = defensive_rating2 * 0.9 + midfield_de_rating2 * 0.1;
						...
						if (team1 >= 0 && team1 < 2) {
							...
							goal1 = Math.floor(
								Math.random() * (Math.floor(team1 / 2) - Math.floor(team1 / 6)) + Math.floor(team1 / 8)
							);
						...
						if (team2 >= 0 && team2 < 2) {
							...
							goal2 = Math.floor(
								Math.random() * (Math.floor(team2 / 2) - Math.floor(team2 / 6)) + Math.floor(team2 / 8)
							);
						...
						for (k = 0; k < goal1; k++) {
							goal_scorer.push(
								team_array_scorer[Math.floor(Math.random() * (team_array_scorer.length - 1))]
							);
						}

						for (k = 0; k < goal2; k++) {
							goal_scorer2.push(
								team_array_scorer2[Math.floor(Math.random() * (team_array_scorer2.length - 1))]
							);
						}
						...
						u[ate].matches.push({
							name1: u[x].team[0].name,
							name2: u[x].team2[0].name,
							score1: goal1,
							score2: goal2,
							team1scorer: goal_scorer,
							team2scorer: goal_scorer2,
							time1: time1,
							time2: time2,
						});
						u[hte].matches.push({
					...
					if (u[x].matches[u[x].matches.length - 1].score2 == 0) {
						...
						for (i = 0; i < 4; i++) {
							for (j = 0; j < u[x].roster.length; j++) {
								if (u[x].pldefender[i].Name == u[x].roster[j].Name) {
									u[x].roster[j].Morale =
										u[x].roster[j].Morale - (u[x].matches[u[x].matches.length - 1].score2 - 2) * 4;
								}
							}
						}
						for (i = 0; i < 5; i++) {
							for (j = 0; j < u[x].roster.length; j++) {
								if (u[x].plmidfield[i].Name == u[x].roster[j].Name) {
									u[x].roster[j].Morale =
										u[x].roster[j].Morale - (u[x].matches[u[x].matches.length - 1].score2 - 2) * 3;
								}
							}
						}
						for (j = 0; j < u[x].roster.length; j++) {
					...
					if (u[x].matches[u[x].matches.length - 1].score1 == 0) {
						...
						for (i = 0; i < 5; i++) {
							for (j = 0; j < u[x].roster.length; j++) {
								if (u[x].plmidfield[i].Name == u[x].roster[j].Name) {
									u[x].roster[j].Morale =
										u[x].roster[j].Morale + (u[x].matches[u[x].matches.length - 1].score1 - 2) * 3;
								}
							}
						}
						for (j = 0; j < u[x].roster.length; j++) {
							if (u[x].plforward[0].Name == u[x].roster[j].Name) {
								u[x].roster[j].Morale =
									u[x].roster[j].Morale + (u[x].matches[u[x].matches.length - 1].score1 - 2) * 4;
							}
						}
					}
